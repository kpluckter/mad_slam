...
def localizeFromMap(x, y, z, h, img, map_path, map_mpp, fov_x, fov_y):
	...
	img_og_rz
	tmpl_warp
	cv2.imwrite("ims/" + str(i) + "og_template.png", tmpl_og_rz)
	cv2.imwrite("ims/" + str(i) + "og_img.png", img_og_rz)
	cv2.imwrite("ims/" + str(i) + "template_warped.png", tmpl_warp)
	...
	####
	# pdb.set_trace()
	if (zm == z) or (xm < 0) or (ym < 0) or (zm < 0) or (zm > 22) or (xm > 50) or (ym > 50):
		return [x_in], [y_in], [z_in], h_in
	else:
		return xm, ym, zm, hm
...
def test():
	...
	rospy.init_node('map_align', anonymous=True)
	rate = rospy.Rate(1)
	pub = rospy.Publisher('/corrected_pose', Twist, queue_size=1)
	rospy.Subscriber("/curr_pose", Twist, poseCallback)
	rospy.Subscriber("/pinhole/image_raw", Image, imageCallback)
	msg = Twist()
	global bridge
	bridge = CvBridge()
	global x,y,z,h,i
	...
	while not rospy.is_shutdown():
		...
		if img_w > 0 and img_h > 0:
			xm, ym, zm, hm = localizeFromMap(x, y, z, h, IMG, map_path, map_mpp, fov_x, fov_y)
			msg.linear.x = xm[0]
			msg.linear.y = ym[0]
			# msg.linear.z = zm[0]
			msg.angular.z = hm
			msg.angular.x = iter_pub
			x = xm[0]
			y = ym[0]
			# z = zm[0]
			h = hm
