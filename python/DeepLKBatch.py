...
def warp_hmg(img, p):
	...
	Y_warp = Y_warp.view(batch_size,h,w) + (h-2)/2
...
class DeepLK(nn.Module):
	...
	def forward(self, img, temp, init_param=None, tol=1e-3, max_itr=500, conv_flag=0):
		...
		while (float(dp.norm(p=2,dim=1,keepdim=True).max()) > tol or itr == 1) and (itr <= max_itr):
			...
			#### --- Convergence criteria code

			# mask_resh = mask.view(batch_size, k * h * w, 1)

			# r_sq_dist = (r ** 2).sum(dim=1,keepdim=True) / mask_resh.sum(dim=1,keepdim=True)

			# if (itr % 1 == 0) and (itr > 1):
			# 	# if (float(r_sq_dist) >  float(r_sq_dist_old)) & (itr > 1):
			# 	print('i: {} dp: {:.3f} r sq dist diff: {:.5f} corner loss: {:.3f}'.format(itr,float(dp.norm()),float(r_sq_dist-r_sq_dist_old),float(sqrt(evaluate.corner_loss(p,param)/4))))

			# # if (float(r_sq_dist-r_sq_dist_old) < 0) and (float(r_sq_dist-r_sq_dist_old) > -0.01):
			# if abs(float(r_sq_dist-r_sq_dist_old)) < 1e-2:
			# 	print('\tr sq dist diff threshold met')

			# if float(dp.norm()) < 1e-2:
			# 	print('\tdp threshold met')

			# r_sq_dist_old = r_sq_dist

			#### --- End convergence criteria code
